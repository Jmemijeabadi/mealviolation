...
from compliance.reporting import build_location_coverage_summary, build_review_summary


SNAPSHOT_SCHEMA_VERSION = "1.1"
SUPPORTED_SNAPSHOT_SCHEMA_VERSIONS = {"1.0", "1.1"}
...
def create_snapshot_bytes(
    ...
    *,
    ...
    payload = {
        ...
        "punch_errors": _records(bundle.punch_errors),
        "meals": _records(bundle.meals),
        "coverage": _records(bundle.coverage),
        "data_quality": _records(bundle.data_quality),
        "reconciliation": _records(bundle.reconciliation),
        "location_summary": _records(
            build_location_coverage_summary(
                bundle.coverage,
                bundle.raw_timecards,
                selected_locations=(context or {}).get("selected_locations", []),
            )
        ),
        "review_summary": _records(build_review_summary(bundle.reviews)),
    }
    return json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8")



def create_executive_snapshot_bytes(
    bundle: AnalysisBundle,
    *,
    app_version: str,
    context: dict[str, Any] | None = None,
) -> bytes:
    """Create an aggregate snapshot without employee names, payroll IDs or punches."""
    context = context or {}
    candidates = bundle.candidates.copy()
    candidate_by_location: list[dict[str, Any]] = []
    candidate_by_reason: list[dict[str, Any]] = []

    if not candidates.empty:
        candidate_source = candidates.copy()
        candidate_source["_workday_key"] = (
            candidate_source.get("Employee Key", "").astype(str)
            + "|"
            + candidate_source.get("Legal Workday Date", "").astype(str)
        )
        location_group = candidate_source.groupby(
            ["Location Ref", "Location"], dropna=False
        ).agg(
            candidate_findings=("Candidate Violation", "size"),
            candidate_workdays=("_workday_key", "nunique"),
            preliminary_exposure=("Premium Estimate", "sum"),
        )
        candidate_by_location = _records(
            location_group.reset_index().rename(
                columns={
                    "candidate_findings": "Candidate Findings",
                    "candidate_workdays": "Candidate Workdays",
                    "preliminary_exposure": "Preliminary Exposure",
                }
            )
        )
        reason_group = (
            candidates.groupby("Candidate Violation", dropna=False)
            .size()
            .rename("Count")
            .reset_index()
        )
        candidate_by_reason = _records(reason_group)

    payload = {
        "schema_version": SNAPSHOT_SCHEMA_VERSION,
        "snapshot_type": "executive_anonymized",
        "app_version": app_version,
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "context": {
            "date_label": context.get("date_label", ""),
            "location_refs": context.get("location_refs", []),
        ...
        "candidate_by_location": candidate_by_location,
        "candidate_by_reason": candidate_by_reason,
        "review_summary": _records(build_review_summary(bundle.reviews)),
        "security": {
            "employee_names_included": False,
            "payroll_ids_included": False,
            "pay_rates_by_employee_included": False,
            "raw_timecards_included": False,
...
def load_snapshot_bytes(data: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("schema_version") not in SUPPORTED_SNAPSHOT_SCHEMA_VERSIONS
    ):
