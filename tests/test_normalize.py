...
def test_completed_clock_out_without_status_is_not_labeled_still_clocked_in() -> None:
    payload = {
        "locRef": "8",
        "businessDates": [
            {
                ...
                "timeCardDetails": [
                    {
                        ...
                        "shftType": 0,
                        "clkInLcl": "2026-07-01T08:00:00",
                        "clkOutLcl": "2026-07-01T14:00:00",
                    }
                ],
            }
        ],
    }
    df = normalize_timecards([payload])
    assert df.iloc[0]["clock_out_status_label"] == "Clock Out Status Missing"
